...
int main() {
  ...
  while (!(std::cin >> numBoids) || numBoids <= 0 ||
         numBoids > maxNumBoids)  // checks if the type is correct, if the type
                                  // is double takes only the integer part
  {
    std::cout << "Invalid input. Please enter a positive integer or reduce the "
                 "value: "
              << std::endl;
    std::cin.clear();
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(),
                    '\n');  // if there are some wrong characters it ignores
                            // them for the next input
  }
  std::cin.ignore(std::numeric_limits<std::streamsize>::max(),
                  '\n');  // if u insert a double the number after inteferes so
                          // this ignores another time also after the cycle ends
  ...
  while (!(std::cin >> time) || time <= 0 || time > maxSimTime) {
    std::cout
        << "Invalid input. Please enter a positive double or reduce the value: "
        << std::endl;
    std::cin.clear();
    std::cin.ignore(std::numeric_limits<std::streamsize>::max(), '\n');
  }
